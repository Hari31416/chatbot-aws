...
*The JWT `IdToken` is saved locally in `localStorage` under `auth_id_token` and automatically attached as `Authorization: Bearer <idToken>` for all chat operations.*
