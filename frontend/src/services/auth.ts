 ...
 */
async function callCognito(target: string, payload: Record<string, any>): Promise<any> {
  ...
  if (!response.ok) {
    throw new Error(data.message || `Cognito auth failed with status ${response.status}`)
  }
  return data
}
 ...
 */
export async function confirmSignUpUser(email: string, code: string): Promise<boolean> {
  if (!clientId) {
    throw new Error("VITE_COGNITO_CLIENT_ID environment variable is missing.")
  }
...
export async function signInUser(email: string, password: string): Promise<AuthSession> {
  ...
  if (!authResult) {
    throw new Error("InitiateAuth did not return credentials. Check password complexity.")
  }
