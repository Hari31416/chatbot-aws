...
export function DocumentsModal({
  ...
  const [activeTab, setActiveTab] = React.useState<'catalog' | 'ingest'>('catalog')
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl w-full max-w-4xl h-[85vh] md:h-[75vh] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 text-zinc-800 dark:text-zinc-100">
        ...
        <div className="flex items-center justify-between border-b border-zinc-150 dark:border-zinc-800 px-6 py-4 bg-zinc-50/50 dark:bg-zinc-900/50">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-bold text-base leading-tight">Knowledge Base Library</h2>
              <p className="text-xs text-zinc-400 font-medium">Manage and ingest documents for context-aware RAG queries</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-zinc-50/20 dark:bg-zinc-950/20">
            ...
            <div className="space-y-4 h-full flex flex-col">
              ...
              <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
                ...
                <div className="text-xs font-semibold text-zinc-450 dark:text-zinc-400 flex items-center gap-1.5">
                  ...
                  <span>Total database footprint: {documents.reduce((acc, d) => acc + d.chunks_ingested, 0)} text vectors</span>
                </div>
              </div>
              ...
              <div className="flex-1 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden bg-white dark:bg-zinc-900 shadow-xs">
                {filteredDocuments.length === 0 ? (
                  <div className="h-64 flex flex-col items-center justify-center text-center p-6">
                    ...
                    <p className="text-sm font-semibold text-zinc-650 dark:text-zinc-300">No documents found</p>
                    <p className="text-xs text-zinc-450 mt-1 max-w-xs">
                      {searchQuery ? 'Try adjusting your search criteria.' : 'Begin by ingesting text or markdown files to populate your AI Knowledge Base.'}
                    </p>
                    {!searchQuery && (
                      <Button
                        onClick={() => setActiveTab('ingest')}
                          ...
                          >
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-2.5">
                                ...
                                <span className="font-semibold text-zinc-800 dark:text-zinc-100 truncate max-w-xs md:max-w-sm" title={doc.source_doc}>
          ...
          {activeTab === 'ingest' && (
            <div className="max-w-2xl mx-auto space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => document.getElementById('rag-file-picker')?.click()}
                    >
                      Browse Files
                    </Button>
                  </div>
                    ...
                    <div className="flex items-center justify-between p-3.5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-xl animate-in slide-in-from-top-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="text-left min-w-0">
                          <p className="text-xs font-semibold text-zinc-700 dark:text-zinc-200 truncate" title={selectedFile.name}>
                            {selectedFile.name}
                          </p>
              ...
              {ingestMode === 'text' && (
                <form onSubmit={handlePasteSubmit} className="space-y-4 text-left">
                  ...
                  <div className="flex justify-end pt-2">
                    <Button
                      type="submit"
                      disabled={isSubmitting || !pasteFilename.trim() || !pasteContent.trim()}
                    >
                      {isSubmitting ? 'Ingesting...' : 'Ingest Document'}
                    </Button>
                  </div>
              ...
              <div className="p-4 rounded-2xl border border-zinc-150 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/30 flex gap-3 text-left">
                ...
                <div className="space-y-1">
                  <h4 className="text-xs font-bold text-zinc-700 dark:text-zinc-200">How does this help my queries?</h4>
                  <p className="text-[11px] text-zinc-450 dark:text-zinc-400 leading-normal">
                    Ingested files are partitioned into semantic overlap chunks and converted to high-dimensional embedding vectors. 
                    When you toggle <strong>RAG</strong> mode in the chat interface and select these documents, relevant sections will be automatically loaded as grounding context to produce extremely accurate answers.
                  </p>
                </div>
              </div>
        ...
        <div className="border-t border-zinc-200 dark:border-zinc-800 px-6 py-4 bg-zinc-50/50 dark:bg-zinc-900/50 flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-zinc-400 text-[10px] font-semibold">
            ...
            <span>Automatic semantic chunking size: 800 chars (overlap 80 chars)</span>
          </div>
